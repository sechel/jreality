...
public class CameraPathExample {
	...
	public static void main(String[] args) {
		...
		Timer movepoint = new Timer(20, new ActionListener() {
			int count = 0;
	 ...
	 public static double[] discreteTorusKnot(double R, double r, int n, int m, double t)	{
			double angle = ( t * 2.0 * Math.PI);
			double a = m * angle, A = n * angle;
			double C = Math.cos(A),			S = Math.sin(A);
			double c = r*Math.cos(a), 			s = r*Math.sin(a);
			return new double[]{C * (R + c), s, S * (R+c)};
	}
