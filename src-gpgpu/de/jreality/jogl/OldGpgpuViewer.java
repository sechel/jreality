...
public class OldGpgpuViewer extends Viewer {
  ...
  public OldGpgpuViewer() {
    super();
  }

  public OldGpgpuViewer(boolean foo) {
    this();
  ...
  private void initPrograms(GL gl) {
    if (recompilePrograms) {
      ...
      try {
        ...
        LineNumberReader lnr = new LineNumberReader(Input.getInput(OldGpgpuViewer.class.getResource("biot_savart-impl.glsl")).getReader());
        for (String line=lnr.readLine(); line != null; line=lnr.readLine()) biotSavart += line+"\n";
        lnr.close();
        
        String rk = "\n";
        // read RK-1
        lnr = new LineNumberReader(Input.getInput(OldGpgpuViewer.class.getResource("K1.glsl")).getReader());
        ...
        rk = "\n";
        // read RK-2
        lnr = new LineNumberReader(Input.getInput(OldGpgpuViewer.class.getResource("K2.glsl")).getReader());
        for (String line=lnr.readLine(); line != null; line=lnr.readLine()) rk += line+"\n";
        ...
        if (forthOrder) {
          rk = "\n";
          // read RK-3
          lnr = new LineNumberReader(Input.getInput(OldGpgpuViewer.class.getResource("K3.glsl")).getReader());
          for (String line=lnr.readLine(); line != null; line=lnr.readLine()) rk += line+"\n";
          progK3 = new GlslProgram(new Appearance(), "foo", null, cst+rk+biotSavart);
  
          rk = "\n";
          // read RK-4
          lnr = new LineNumberReader(Input.getInput(OldGpgpuViewer.class.getResource("K4.glsl")).getReader());
          for (String line=lnr.readLine(); line != null; line=lnr.readLine()) rk += line+"\n";
          progK4 = new GlslProgram(new Appearance(), "foo", null, cst+rk+biotSavart);
  
          rk = "\n";
          // read RK-merge
          lnr = new LineNumberReader(Input.getInput(OldGpgpuViewer.class.getResource("RK4.glsl")).getReader());
          for (String line=lnr.readLine(); line != null; line=lnr.readLine()) rk += line+"\n";
          progMerge = new GlslProgram(new Appearance(), "foo", null, rk);
        } else {
          rk = "\n";
          // read RK-merge
          lnr = new LineNumberReader(Input.getInput(OldGpgpuViewer.class.getResource("RK2.glsl")).getReader());
          for (String line=lnr.readLine(); line != null; line=lnr.readLine()) rk += line+"\n";
          progMerge = new GlslProgram(new Appearance(), "foo", null, rk);
        }
