 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class JOGLRendererHelperNew {
	...
	public static void setupSphereDLists(JOGLRendererNew jr)	{
	 ...
	 * @return
	 */
	public static int getSphereDLists(int i, JOGLRendererNew jr) {
...
//		}
	 ...
	 * @param sg
	 */
	public static void drawLines(IndexedLineSet sg, GLCanvas theCanvas,JOGLRendererNew.JOGLPeerComponent jpc, boolean pickMode) {
		GL gl = theCanvas.getGL();
...
//		}
	...
	static void processLights(SceneGraphComponent theRoot, GL globalGL, boolean lightListDirty) {
		...
		for (int i = 0; i<n; ++i)	{
			...
			if (light instanceof SpotLight)		wisit((SpotLight) light, globalGL);
			else if (light instanceof PointLight)		wisit((PointLight) light, globalGL);
			else if (light instanceof DirectionalLight)		wisit((DirectionalLight) light,globalGL);
	...
	public static void wisit(SpotLight dl, GL globalGL)		{
		  if (lightCount >= GL.GL_LIGHT7)	{
		  	System.out.println("Max. # lights exceeded");
		  	return;
		  }
