...
public class DomeViewer extends Viewer {
	...
	private String[] name = new String[]{"texFront", "texRight", "texBack", "texLeft", "texTop", "texFloor"};
	
	private int textureUnit[] = new int[]{GL3.GL_TEXTURE0, GL3.GL_TEXTURE1, GL3.GL_TEXTURE2, GL3.GL_TEXTURE3, GL3.GL_TEXTURE4, GL3.GL_TEXTURE5};
	...
	public void init(GLAutoDrawable gla){
        ...
        for(int i = 0; i < 6; i++){
        	...
        	//gl.glTexImage2D(gl.GL_TEXTURE_2D, 0, gl.GL_RGB, 512, 512, 0, gl.GL_RGB, gl.GL_BYTE, null);
            ...
            gl.glFramebufferTexture2D(gl.GL_FRAMEBUFFER, gl.GL_COLOR_ATTACHMENT0, gl.GL_TEXTURE_2D, texs[i], 0);
            //attach renderbuffer
    		gl.glFramebufferRenderbuffer(gl.GL_FRAMEBUFFER, gl.GL_DEPTH_ATTACHMENT, gl.GL_RENDERBUFFER, rbuffer[i]);
        }
    	...
    	float[] vertdata = new float[]{-1, 1, 0, 1, 1, 0, 1, -1, 0, -1, 1, 0, 1, -1, 0, -1, -1, 0};
...
//        	  gl.glBindTexture(gl.GL_TEXTURE_2D, texs[i]);
//        	  gl.glUniform1i(gl.glGetUniformLocation(shader.shaderprogram, name[i]), i);
//        }
        super.init(gla);
	...
	public void display(GLAutoDrawable gla){
		...
		for(int i = 0; i < 6; i++){
			...
			switch(i){
			case 0: axis = new double[]{0,1,0}; angle = 0;
					break;
			case 1: axis = new double[]{0,1,0}; angle = -Math.PI/2;
					break;
			case 2: axis = new double[]{0,1,0}; angle = Math.PI;
					break;
			case 3: axis = new double[]{0,1,0}; angle = Math.PI/2;
					break;
			case 4: axis = new double[]{1,0,0}; angle = Math.PI/2;
					break;
			case 5: axis = new double[]{1,0,0}; angle = -Math.PI/2;
					break;
			}
			...
			SceneGraphComponent sgc = (SceneGraphComponent)sgp.get(sgp.getLength()-2);
...
//			super.display(gla, width, height);
		...
		//gl.glViewport(0, 0, renderer.theCanvas.getWidth(), renderer.theCanvas.getHeight());
		gl.glViewport(0, 0, (int)Math.round(getViewingComponentSize().getWidth()), (int)Math.round(getViewingComponentSize().getHeight()));
        ...
        gl.glUniform1f(gl.glGetUniformLocation(shader.shaderprogram, "angle"), FOV);
