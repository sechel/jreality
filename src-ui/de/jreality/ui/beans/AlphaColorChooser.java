...
public class AlphaColorChooser extends JPanel implements ChangeListener {
    ...
    public AlphaColorChooser(Color color, boolean withSamples, boolean withRGB, boolean withHSV) {
        ...
        int numPanels = 3;
        if (!withSamples) {
        	colorChooser.removeChooserPanel(colorChooser.getChooserPanels()[numPanels - 3]);
        	numPanels--;
        }
        if (!withRGB) {
        	colorChooser.removeChooserPanel(colorChooser.getChooserPanels()[numPanels - 2]);
        	numPanels--;
        }
        if (!withHSV) {
        	colorChooser.removeChooserPanel(colorChooser.getChooserPanels()[numPanels -1]);
        }
